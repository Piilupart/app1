...
template = """
 You are a marketing copywriter with 20 years of experience. You are analyzing customer's background to write personalized product description that only this customer will receive; 
    ...
    BENEFITS: describe in 3 sentences why this product is perfect considering customers age group and sport;
    USE CASE: write a story in 5 sentences, of an example weekend activity taking into account sport {sport} and age {agegroup}, write a story in first person, example "I started my Saturday morning with ...";
"""

prompt = PromptTemplate(
    input_variables=["agegroup", "sport", "content"],
    template=template,
)
...
def get_hobby():
    input_text = st.text_input(label="Customers main sport", key="hobby_input")
    return input_text
...
#        st.stop()
    ...
    prompt_with_content = prompt.format(agegroup=option_agegroup, sport=hobby_input, content=content_input)
